      ...
      <div className="flex gap-4 h-screen overflow-hidden">
        <div className="w-64 border-r border-border overflow-y-auto flex-shrink-0">
          <div className="sticky top-0 bg-background p-3 border-b border-border">
            <h3 className="font-semibold text-sm text-foreground">Vendor Credits</h3>
          </div>
          <nav className="space-y-1 p-2">
            {mockCredits.map((c) => (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground truncate">{c.vendorName}</div>
              </button>
            ))}
          </nav>
        </div>
          ...
          <div className="flex-shrink-0 border-b border-border p-4">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-bold text-foreground">{currentCredit.creditNumber}</h1>
                  <p className="text-xs text-muted-foreground">{currentCredit.vendorName}</p>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => navigate(`/purchases/vendor-credits/${currentCredit.id}`)} className="p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors" title="Edit"><Edit size={16} /></button>
              ...
              <Card className="p-8 bg-white">
                <div className="space-y-6">
                  <div className="flex justify-between items-start">
                    <div>
                      <h2 className="text-2xl font-bold text-foreground">Vendor Credit</h2>
                      <p className="text-sm text-muted-foreground">#{currentCredit.creditNumber}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">Status</p>
                      <p className="font-medium text-foreground">{currentCredit.status}</p>
                    </div>
                  </div>

                  <div>
                    <p className="text-sm font-semibold text-foreground mb-2">Vendor:</p>
                    <p className="text-sm text-foreground">{currentCredit.vendorName}</p>
                  </div>
                  ...
                  <div className="flex justify-end">
                    <div className="w-48">
                      <div className="flex justify-between py-2 border-t-2 border-foreground/20">
                        ...
                        <span className="font-bold text-foreground">${currentCredit.amount?.toLocaleString?.() ?? ""}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {currentCredit.notes && (
                    <div className="pt-4 border-t border-foreground/10">
                      ...
                      <p className="text-sm text-foreground">{currentCredit.notes}</p>
                    </div>
                  )}
                </div>
              </Card>
