...
export default function QuoteDetail() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState(
    currentQuote?.template || "professional"
  );
  ...
  const displayItems = (
    (currentQuote as any).cart_items ??
    (currentQuote as any).items ??
    []
  ).map((it: any) =>
    it.item_name
      ? {
  ...
  const handleQuoteChange = (quoteId: string) => {
    ...
    if (quote) {
      ...
      setSelectedTemplate((quote as any).template || "professional");
      navigate(`/sales/quotes/view/${quoteId}`);
    }
  };
          ...
          <div className="flex-shrink-0 border-b border-border p-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    (window.location.href = `mailto:${currentQuote.email}`)
                  }
                ...
                <button
                  onClick={() => navigate(`/sales/quotes/${currentQuote.id}`)}
                ...
                {isEditingTemplate ? (
                  <div className="grid gap-3 md:grid-cols-3">
                    {templates.map((template) => (
                      ...
                      >
                        <div
                          className={`h-32 rounded ${getTemplatePreview()} mb-2`}
                        />
                  ...
                  <div className="flex justify-end">
                    <div className="w-48">
                      <div className="flex justify-between py-2 border-t-2 border-foreground/20">
                        ...
                        <span className="font-bold text-foreground">
                          ${currentQuote.amount.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <Card className="p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <p
                      className={`text-sm font-medium ${currentQuote.status === "Accepted" ? "text-accent" : "text-foreground"}`}
                    >
                      {currentQuote.status}
                    </p>
                  </div>
