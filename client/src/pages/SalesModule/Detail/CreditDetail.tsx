      ...
      <div className="flex gap-4 h-screen overflow-hidden">
        <div className="w-64 border-r border-border overflow-y-auto flex-shrink-0">
          <div className="sticky top-0 bg-background p-3 border-b border-border">
            <h3 className="font-semibold text-sm text-foreground">Credits</h3>
          </div>
          <nav className="space-y-1 p-2">
            {list.map((it) => (
              <button
                key={it.id}
                onClick={() => navigate(`/sales/credits/view/${it.id}`)}
              ...
              >
                <div className="font-semibold">{it.creditNumber}</div>
          ...
          <div className="flex-shrink-0 border-b border-border p-4">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <button
                  onClick={() => navigate("/sales/credits")}
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => navigate(`/sales/credits/${current.id}`)}
                ...
                <Card className="p-4">
                  ...
                  <div className="max-h-72 overflow-y-auto rounded">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {((current as any).cart_items || []).map((it: any) => (
                          <tr
                            key={it.id}
